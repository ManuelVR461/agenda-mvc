...
class mysql{
	...
	*/
	private function conectar(){
		...
		if(!$this->link){
			throw new Excepcion("Error: No pudo conectarse al servidor de Base de Datos ".$this->servidor, mysqli_connect_error());
		}
	}
    ...
    */
    public function query($sql){
		...
		mysqli_query($this->link, "SET CHARSET 'UTF8'");
		$query = mysqli_query($this->link, $sql);
		if(!$query){
			throw new Excepcion("Error: [MySQL Error] Hubo un error al procesar su consulta. ", mysqli_error($this->link));
		}
		$this->data = $query;
		@mysqli_next_result($this->link);//@TODO
		return $this->data;
	}

	/** Inserta, Actualiza o Borra una o más filas de acuerdo a lo expresado en $sql. Devuelve el id para mysql.
	 * @param String $sql Cadena con el texto de la query a ejecutar
	 * @param Boolean $returnId indica si devuelve
	 * @return Int $this->last_id id
	 *
	 * @author Jorge Andrade M.
	 */
    public function edit($sql ,$returnId=true){
		if($this->query($sql)){
			if($returnId){
	...
	*/
	public function getRow($data){
		if($data){
			$this->array = @mysqli_fetch_assoc($data);
			@mysqli_next_result($this->link);//@TODO
		}
		if($this->array){
	...
	*/
	public function __destruct(){
		// mysqli_next_result($this->link);//@TODO
		// @mysqli_free_result($this->data); //@TODO
