...
class sybase{
	...
	*/
	private function conectar(){
		...
		$this->link = @sybase_connect($this->servidor,$this->usuario,$this->password,$this->charset,APP_NAME);
		if(!$this->link){
			throw new Excepcion("Error: No pudo conectarse al servidor de Base de Datos ".$this->servidor, sybase_get_last_message());
		}
		$handler = new Model();
		sybase_set_message_handler(array($handler,'msg_handler'));
		sybase_select_db($this->base_datos, $this->link);
	}
    ...
    */
    public function query($sql){
		...
		$query = @sybase_query($sql,$this->link);
		if(!$query){
			throw new Excepcion("Error: [Sybase Error] Hubo un error al procesar su consulta.", sybase_get_last_message());
		}
		$this->data = $query;
		return $this->data;
	}
	...
	*/
	public function getRow($data){
		if($data){
			$this->array = sybase_fetch_assoc($data);
		}
		if($this->array){
	...
	*/
	public function count($sql){
		return sybase_num_rows($this->query($sql));
	}
